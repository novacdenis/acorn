...
export function useDebouncedCallback<T extends (...args: Parameters<T>) => ReturnType<T>>(
  ...
  const debounced = useMemo(() => {
    const invokeFunc = (time: number) => {
      const args = lastArgs.current as Parameters<T>;
    ...
    const timerExpired = () => {
      ...
      const remainingWait = maxing
        ? Math.min(timeWaiting, Number(maxWait) - timeSinceLastInvoke)
        : timeWaiting;
    ...
    const func: DebouncedState<T> = (...args: Parameters<T>): ReturnType<T> | undefined => {
      ...
      if (isInvoking) {
        if (!timerId.current && mounted.current) {
          ...
          startTimer(timerExpired, Number(wait));
    ...
    func.cancel = () => {
      if (timerId.current) {
        useRAF ? cancelAnimationFrame(timerId.current as number) : clearTimeout(timerId.current);
      }
